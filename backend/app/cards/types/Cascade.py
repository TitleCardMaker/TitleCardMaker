...
class CascadeTitleCard(BaseCardType):
    ...
    API_DETAILS = CardTypeDescription(
        name='Cascade',
        identifier='cascade',
        example='/public/cards/cascade.webp',
...
def get_validator_model() -> type[BaseCardModel]:
    ...
    class CardModel(BaseCardTypeAllText):
        ...
        cascade_count: Annotated[
            int,
            Field(ge=0, le=25)
        ] = CascadeTitleCard.DEFAULT_CASCADE_COUNT
        ...
        cascade_width: Annotated[
            int,
            Field(ge=0, le=50)
        ] = CascadeTitleCard.DEFAULT_CASCADE_WIDTH
        ...
        def assign_unassigned_color(self) -> Self:
            ...
            if self.alt_text_color is None:
                self.alt_text_color = self.episode_text_color

            return self

        @model_validator(mode='after')
        def validate_sequence_strings(self) -> Self:
            """
