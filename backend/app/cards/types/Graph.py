...
class GraphTitleCard(BaseCardType):
    ...
    API_DETAILS = CardTypeDescription(
        name='Graph',
        identifier='graph',
        example='/public/cards/graph.webp',
...
def get_validator_model() -> type[BaseCardModel]:
    ...
    class CardModel(BaseCardModel):
        ...
        font_vertical_shift: int = 0
        graph_text_font_size: FontSize | None = None
        grayscale: bool = False
        ...
        def validate_extras(self) -> Self:
            ...
            if self.graph_width > self.graph_radius:
                self.graph_width = self.graph_radius

            # Scale episode text size by radius if not provided
            if self.graph_text_font_size is None:
                self.graph_text_font_size = \
                    self.graph_radius / GraphTitleCard.GRAPH_RADIUS

            # Episode text formatted as {nom} / {den}; calculate percentage
            if (_match := graph_episode_text_regex.match(self.episode_text)):
                self.percentage = int(_match[1]) / int(_match[2])
