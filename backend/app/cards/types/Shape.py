...
class ShapeTitleCard(BaseCardType):
    ...
    API_DETAILS = CardTypeDescription(
        name='Shape',
        identifier='shape',
        example='/public/cards/shape.webp',
...
def get_validator_model() -> type[BaseCardModel]:
    ...
    class CardModel(BaseCardTypeAllText):
        ...
        def format_title_text(self) -> Self:
            """
            Add the episode text to the beginning of the title text.
            """
            # Add episode text before title text if not hiding
            if self.hide_episode_text is not True:
                self.title_text = f'{self.episode_text} {self.title_text}'
            return self
        ...
        def assign_unassigned_color(self) -> Self:
            ...
            if self.season_text_color is None:
                self.season_text_color = self.shape_color
            return self

    return CardModel


add_card_cli(__name__, ShapeTitleCard, get_validator_model())
