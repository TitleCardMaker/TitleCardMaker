...
class Font(YamlReader):
    ...
    @staticmethod
    def validate_yaml(yaml: dict[str, Any] | Any) -> bool:
        """
        Validate the given font YAML.

        Args:
            yaml: Font map YAML to validate.
        ...
        if not isinstance(yaml, dict):
            log.error(f'Font is invalid')
            return False

        # Validate all given attributes
        for key in yaml.keys():
            if key not in Font.VALID_ATTRIBUTES:
                log.error(f'Font "{key}" is invalid')
                return False
