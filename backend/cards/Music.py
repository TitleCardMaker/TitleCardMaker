...
def get_validator_model() -> type[Base]:
    ...
    class CardModel(BaseCardTypeCustomFontAllText):
        ...
        def assign_unassigned_player_action(self) -> Self:
            ...
            if self.pause_or_play == 'watched':
                # No watched status, use default
                if self.watched is None:
                    self.pause_or_play = MusicTitleCard.DEFAULT_PLAYER_ACTION
                else:
                    self.pause_or_play  = 'pause' if self.watched else 'play'

            return self

        @model_validator(mode='after')
        def validate_player_width(self) -> Self:
            """
            Verify the player width is not less than 600 if the controls
            are enabled.
            """

            if self.add_controls and self.player_width < 600:
                raise ValueError(
        ...
        def truncate_long_titles_(self) -> Self:
            ...
            if (truncate := self.truncate_long_titles) != 'False':
                if len(lines := self.title_text.splitlines()) > truncate:
                    self.title_text = '\n'.join(lines[:truncate]) + ' ...'
                if len(self.season_text) > 3:
                    self.season_text = self.season_text[:3] + '..'
                if len(self.episode_text) > 3:
                    self.episode_text = self.episode_text[:3] + '..'
