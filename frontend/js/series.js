...
function getStatistics() {
  $.ajax({
    type: 'GET',
    url: '/api/v2/statistics/series/{{ series.id }}',
    /**
...
function createEpisodeCard(episodeId) {
  ...
  $.ajax({
    type: 'POST',
    url: `/api/v2/cards/episode/${episodeId}`,
    success: () => {
...
function saveEpisodeConfig(episodeId) {
  ...
  $.ajax({
    type: 'PATCH',
    url: `/api/v2/episodes/episode/${episodeId}`,
...
function refreshEpisodes() {
  ...
  $.ajax({
    type: 'POST',
    url: '/api/v2/episodes/series/{{ series.id }}/refresh',
    success: () => {
...
async function editEpisodeExtras(episodeId, allEpisodeIds) {
  ...
  $('#episode-extras-form').toggleClass('loading', true);

  // Begin querying Episode data from API
  const episodeData = fetch(`/api/v2/episodes/episode/${episodeId}`)
  ...
  const assignButtonNavs = (epId) => {
    $('#episode-extras-modal [data-action="previous-episode"]')
      .off('click')
      .on('click', () => {
        const previousEpisode = allEpisodeIds[allEpisodeIds.indexOf(epId) - 1];
        if (previousEpisode) {
          editEpisodeExtras(previousEpisode, allEpisodeIds);
        } else {
    ...
    $('#episode-extras-modal [data-action="next-episode"]')
      .off('click')
      .on('click', () => {
        const nextEpisode = allEpisodeIds[allEpisodeIds.indexOf(epId) + 1];
        if (nextEpisode) {
          editEpisodeExtras(nextEpisode, allEpisodeIds);
        } else {
          showInfoToast('No next Episode on current page');
        }
      });
  };
  assignButtonNavs(episodeId);

  // Await response from API
  /** @type {Episode} */
  const episode = await episodeData.then(resp => resp.json());
  ...
  $('#episode-extras-form')
    ...
    .on('submit', /** @param {SubmitEvent} event */ event => {
      ...
      $.ajax({
        type: 'PATCH',
        url: `/api/v2/episodes/episode/${episode.id}`,
        // Convert empty extra object to null
        data: JSON.stringify({ translations, extras: Object.keys(extras).length === 0 ? null : extras }),
        contentType: 'application/json',
        /**
         * Edit successful, show toast.
         * @param {Episode} updatedEpisode - Newly edited Episode.
         */
        success: updatedEpisode => showInfoToast('Updated Episode'),
        error: response => showErrorToast({title: 'Error Updating Episode', response}),
...
function uploadEpisodeSource(episodeId) {
  $('#upload-source-form').off('submit').on('submit', event => {
    ...
    $.ajax({
      type: 'PUT',
      url: `/api/v2/sources/episode/${episodeId}/upload`,
...
function mirrorSourceImage(episodeId) {
  ...
  $.ajax({
    type: 'PUT',
    url: `/api/v2/sources/episode/${episodeId}/mirror`,
    success: () => {
...
function deleteEpisodeMaskImage(episodeId) {
  $.ajax({
    type: 'DELETE',
    url: `/api/v2/sources/episode/${episodeId}/mask`,
    success: () => {
...
function deleteSourceImage(episodeId) {
  ...
  $.ajax({
    type: 'DELETE',
    url: `/api/v2/sources/episode/${episodeId}`,
...
function resetMaskEditor() {
  ...
  document.querySelector('#mask-editor-modal .image-compare').style.display = '';
  // Hide the mask editor canvas
  document.querySelector('#edit-mask-canvas').style.display = 'none';
  // Disable edit and save buttons
  document.querySelector('#mask-editor-modal .button[data-action="edit-mask"]').classList.add('disabled');
  document.querySelector('#mask-editor-modal .button[data-action="save-mask"]').classList.add('disabled');
}
...
function getBlueprintCount() {
  $.ajax({
    type: 'GET',
    url: '/api/v2/blueprints/query/series/{{ series.id }}?allow_refresh=false',
    /**
...
async function getEpisodeData(page=1) {
  ...
  const rows = episodeData.items.map(episode => {
    ...
    if (!simplified_data_table) {
      ...
      row.querySelector('input[name="font_stroke_width"]').value = episode.font_stroke_width === null ? episode.font_stroke_width : Math.round(episode.font_stroke_width * 100);
    ...
    row.querySelector('td[data-column="extras"] a').onclick = () => editEpisodeExtras(episode.id, episodeData.items.map(ep => ep.id));
    if (!simplified_data_table) {
...
function toggleStatus() {
  ...
  $.ajax({
    type: 'PUT',
    url: '/api/v2/series/series/{{ series.id }}/toggle-monitor',
    /**
...
function processSeries() {
  ...
  $.ajax({
    type: 'POST',
    url: '/api/v2/series/series/{{ series.id }}/process',
...
function importBlueprint(cardId, blueprint, isSet=false) {
  ...
  if (isSet) {
    ...
    $.ajax({
      type: 'POST',
      url: `/api/v2/blueprints/import/blueprint/${blueprint.id}`,
      /**
    ...
    $.ajax({
      type: 'PUT',
      url: `/api/v2/blueprints/import/series/{{ series.id }}/blueprint/${blueprint.id}`,
      /**
...
function exportBlueprint() {
  ...
  $.ajax({
    type: 'GET',
    url: '/api/v2/blueprints/export/series/{{ series.id }}/zip',
...
function downloadLogo(url, seasonNumber=null) {
  ...
  $.ajax({
    type: 'PUT',
    url: '/api/v2/sources/series/{{ series.id }}/logo/upload' + (seasonNumber === null ? '' : `?season_number=${seasonNumber}`),
    data: form,
    cache: false,
    contentType: false,
    processData: false,
...
function downloadSeriesBackdrop(url, seasonNumber=null) {
  ...
  $.ajax({
    type: 'PUT',
    url: '/api/v2/sources/series/{{ series.id }}/backdrop/upload' + (seasonNumber === null ? '' : `?season_number=${seasonNumber}`),
    data: form,
    cache: false,
    contentType: false,
    processData: false,
...
function browseSourceImages(episodeId, cardElementId, episodeIds) {
  ...
  $.ajax({
    type: 'GET',
    url: `/api/v2/sources/episode/${episodeId}/browse`,
    /**
...
function browseLogos(seasonNumber) {
  ...
  $.ajax({
    type: 'GET',
    url: '/api/v2/sources/series/{{ series.id }}/logo/browse',
    /**
...
function browseBackdrops(seasonNumber) {
  ...
  $.ajax({
    type: 'GET',
    url: `/api/v2/sources/series/{{ series.id }}/backdrop/browse`,
    /**
...
function uploadLogo(seasonNumber=null) {
  ...
  $.ajax({
    type: 'PUT',
    url: `/api/v2/sources/series/{{ series.id }}/logo/upload?${params.toString()}`,
    data: form,
    cache: false,
    contentType: false,
    processData: false,
...
function getSourceImages() {
  ...
  $.ajax({
    type: 'POST',
    url: '/api/v2/sources/series/{{ series.id }}',
...
function createTitleCards() {
  ...
  $.ajax({
    type: 'POST',
    url: '/api/v2/cards/series/{{ series.id }}',
    success: () => {
...
function loadCard(cardId) {
  ...
  $.ajax({
    type: 'PUT',
    url: `/api/v2/cards/card/${cardId}/load`,
...
function deleteAllEpisodes() {
  ...
  $.ajax({
    type: 'DELETE',
    url: '/api/v2/episodes/series/{{ series.id }}',
    /**
...
function deleteEpisode(id) {
  $.ajax({
    type: 'DELETE',
    url: `/api/v2/episodes/episode/${id}`,
    success: () => {
...
function navigateSeries(next_or_previous) {
  $.ajax({
    type: 'GET',
    url: `/api/v2/series/series/{{ series.id }}/${next_or_previous}`,
    /**
...
function removePlexLabels(interface_id, library_name) {  
  ...
  $.ajax({
    type: 'DELETE',
    url: `/api/v2/series/series/{{ series.id }}/plex-labels/library?${params}`,
...
function uploadSourceImages() {
  ...
  $.ajax({
    type: 'PUT',
    url: '/api/v2/sources/series/{{ series.id }}/upload',
    data: form,
    cache: false,
    contentType: false,
    processData: false,
...
function importMediuxYaml(event) {
  ...
  $.ajax({
    type: 'POST',
    url: `/api/v2/import/series/{{ series.id }}/cards/mediux?${params.toString()}`,
    data: data,
    contentType: 'application/json',
    success: () => {
...
function uploadCards(setTextless=false) {
  ...
  $.ajax({
    type: 'POST',
    url: `/api/v2/import/series/{{ series.id }}/cards/files?textless=${setTextless}`,
    data: f,
    cache: false,
    contentType: false,
    processData: false,
  ...
  $('.dropdown[data-value="load-episode-library"]').dropdown({
    onChange: function(value, text, $selectedItem) {
        ...
        $(`.field[data-value="episodes"][data-connection-id="${connectionId}"] div.dropdown`)
          .dropdown({
            ...
            onChange: function(value, text, $selectedItem) {
              ...
              $.ajax({
                type: 'PUT',
                url: `/api/v2/cards/card/${cardID}/load?${params.toString()}`,
      ...
      if ((connectionId in connectionEpisodeData) && (libraryName in connectionEpisodeData[connectionId])) {
        ...
        $.ajax({
          type: 'GET',
          url: `/api/v2/episodes/series/{{ series.id }}/connection/${connectionId}?library_name=${libraryName}`,
          /**
...
function editMaskFile(episodeId) {
  const _compare = document.querySelector('#mask-editor-modal .image-compare');
  const [width, height] = [_compare.clientWidth, _compare.clientHeight];
  _compare.style.display = 'none';

  /** @type {HTMLCanvasElement} */
  const canvas = document.querySelector('#mask-editor-modal canvas');
  canvas.width = width; canvas.height = height;

  const applyMask = initializeEditableMask({
    canvas: canvas,
    imageSource: maskSource,
    applyButton: document.querySelector('#mask-editor-modal .button[data-action="apply-selection"]'),
  });

  document.querySelector('#mask-editor-modal .button[data-action="save-mask"]').onclick = () => {
...
function backgroundRemovaltest(episodeId) {
  const params = new URLSearchParams({
    url: 'http://localhost:8002/api/v2/remove',
    episode_id: episodeId || 6388,
    timeout: 60,
  });

  const rembgButton = document.querySelector('#mask-editor-modal .button[data-action="remove-background"] .icon');
  rembgButton.classList.add('spinner', 'loading');
  rembgButton.classList.remove('magic');

  $.ajax({
    url: `/api/v2/webhooks/background-removal?${params.toString()}`,
    type: 'POST',
    // data: formData,
    processData: false, // Important! Prevent jQuery from automatically transforming the data
    contentType: false, // Important! Ensures the correct content type
    ...
    success: function(response) {
      ...
      document.querySelector('#mask-editor-modal .button[data-action="edit-mask"]').classList.remove('disabled');
      document.querySelector('#mask-editor-modal .button[data-action="save-mask"]').classList.remove('disabled');
      document.querySelector('#mask-editor-modal .button[data-action="save-mask"]').onclick = () => {
        $.ajax({
          url: `/api/v2/sources/episode/${episodeId}/mask`,
          type: 'PUT',
